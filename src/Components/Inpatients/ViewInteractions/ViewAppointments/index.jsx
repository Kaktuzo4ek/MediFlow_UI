...
const ViewAppointments = () => {
  ...
  const changeFilterBy = (event) => {
    ...
    else inputReason.classList.remove(styles.visible);
    ...
    switch (filterBy) {
            ...
            (item) =>
              (item.appointments =
                item.appointments.filter(
                  (itemApp) =>
                    (itemApp.appointments = itemApp
                      .filter(
                        ({ appointmentsAndDiagnosesICPC2 }) =>
                          appointmentsAndDiagnosesICPC2.diagnosisICPC2
                            .diagnosisCode +
                          ' ' +
                          appointmentsAndDiagnosesICPC2.diagnosisICPC2
                            .diagnosisName,
                    ...
                    (itemApp.appointments = itemApp
                      .filter(
                        ({ appointmentsAndServices }) =>
                          appointmentsAndServices.service.serviceId +
                          ' ' +
                          appointmentsAndServices.service.serviceName,
                      )
                      .toLowerCase()
                      .includes(filter.toLowerCase())),
                ).length > 0),
      ...
      case 'date':
        setEpisode(
          arrayForFilter.filter(
            (item) =>
              (item.appointments = item.appointments.filter(
                (itemApp) =>
                  (itemApp.appointments = itemApp.filter(({ date }) =>
  ...
  const setDataAndNavigateToViewAppointmentReport = (
    episodeId,
    appointmentId,
  ) => {
        ...
        <div className={styles.flexSelectAndBtn}>
          <div className={styles.flexForSelects}>
            ...
            <input
              ...
              value={filter}
              onChange={changeFilter}
              placeholder='Введіть дату візиту'
            />
          </div>
          <div className={styles.flexButtons}>
            <button
              ...
              onClick={() => filterReferrals(filter, filterEpisode)}
            >
              Пошук
            </button>
            <button
                ...
                <tbody>
                  {itemEpisode.appointments.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        {itemEpisode.diagnosisMKX10AM &&
                          `(${itemEpisode.diagnosisMKX10AM.diagnosisId}) ${itemEpisode.diagnosisMKX10AM.diagnosisName}`}
                      </td>
                      <td>
                        {item.appointmentsAndServices.map(
                          (a, index) =>
                            `(${a.service.serviceId}) ${
                              ...
                              index + 1 !== item.appointmentsAndServices.length
                                ? ', '
                                : ''
                            }`,
                        )}
                            ...
                            >
                              <div className={styles.buttonsWrapper}>
                                <button
                                  ...
                                  type='button'
                                  onClick={() =>
                                    setDataAndNavigateToEditAppointment(
                                      item.appointmentId,
                                    )
                                  }
                                >
