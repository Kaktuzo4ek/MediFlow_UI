...
const ViewReferrals = () => {
  ...
  const changeFilterBy = (event) => {
    ...
    else inputRefId.classList.remove(styles.visible);
    ...
    else inputPriority.classList.remove(styles.visible);
  ...
  const filterReferrals = (filter, arrayForFilter) => {
    switch (filterBy) {
      case 'refId':
        setReferralPackages(
          arrayForFilter.filter(({ referralPackageId }) =>
            referralPackageId.toLowerCase().includes(filter.toLowerCase()),
          ),
        );
        break;
      case 'date':
        setReferralPackages(
          arrayForFilter.filter(({ date }) =>
      ...
      case 'service':
        setReferralPackages(
          arrayForFilter.filter(
            (item) =>
              (item.referrals = item.referrals.filter(({ service }) =>
      ...
      case 'priority':
        setReferralPackages(
          arrayForFilter.filter(
            (item) =>
              (item.referrals = item.referrals.filter(({ priority }) =>
                priority.toLowerCase().includes(filter.toLowerCase()),
              )).length > 0,
          ),
        );
        break;
      ...
      case 'status':
        setReferralPackages(
          arrayForFilter.filter(
            (item) =>
              (item.referrals = item.referrals.filter(({ processStatus }) =>
                processStatus.toLowerCase().includes(filter.toLowerCase()),
              )).length > 0,
          ),
        );
        break;
  ...
  return (
    <div>
            ...
            <div className={styles.flexSelectAndBtn}>
              <div className={styles.flexForSelects}>
                ...
                <input
                  ...
                  value={filter}
                  onChange={changeFilter}
                  placeholder='Введіть дату виписування'
                />
                <input
                  type='text'
                  id='filter_service'
                  ...
                  value={filter}
                  onChange={changeFilter}
                  placeholder='Введіть групу послуг/послугу'
                />
                <input
                  type='text'
                  id='filter_priority'
                ...
                <input
                  ...
                  value={filter}
                  onChange={changeFilter}
                  placeholder='Введіть статус процесу'
                />
              </div>
              <div className={styles.flexButtons}>
                <button
                  ...
                  onClick={() => filterReferrals(filter, referralsFilter)}
                >
                  Пошук
                </button>
                <button
      ...
      <div className={styles.procedureCountBlock}>
        <div className={styles.container}>
          <p
            {...referralPackages.map(
              (item) => (tmpRefCount = tmpRefCount + item.referrals.length),
            )}
          >
            Кількість ({tmpRefCount})
          </p>
        </div>
      </div>
              ...
              ? referralPackages.map((itemPackage, indexPackage) => (
                  <tbody>
                    {itemPackage.referrals.map((item, index) => (
                      <tr
                        key={index}
                        className={isFinished[count] && styles.finishedReferral}
                      >
                        <td>{count++}</td>
                        ...
                        <td>
                          ({item.service.serviceId}) {item.service.serviceName}
                        </td>
                        <td>{item.priority}</td>
                        <td>{item.service.category.categoryName}</td>
                        <td>
                          {itemPackage.doctor.surname} {itemPackage.doctor.name}{' '}
                          {itemPackage.doctor.patronymic}
                        </td>
                        <td>{item.status}</td>
                        <td>{item.processStatus.split(' (')[0]}</td>
                        <td>
                          {' '}
                          {doctorId === itemPackage.doctor.id &&
                          item.processStatus.split(' (')[0] !== 'Погашене' ? (
                            <div className={styles.flexForAction}>
                              <Image
                                ...
                                onClick={() =>
                                  setModalAndData(
                                    ...
                                    item.priority,
                                    item.category,
                                    item.hospitalizationDepartment,
                                  )
                                }
                              />
                              <Image
                              ...
                              <Image
                                ...
                                className={styles.actionBtn}
                                onClick={() =>
                                  setDataAndNavigateToViewReferral(
                                    item.referralId,
                                    item.referralPackageId,
                                  )
                                }
      ...
      <EditReferralModal
        ...
        referalId={referralIdModal}
        service={serviceObj}
        priority={priorityObj}
        category={categoryObj}
        hospDep={hospDepObj}
        updateTable={getReferrals}
        isOpen={isOpen}
