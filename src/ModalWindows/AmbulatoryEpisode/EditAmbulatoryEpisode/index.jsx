...
const EditEpisodeModal = props => {
    ...
    const updateEpisode = () => {
        axios({
            method: 'put',
            url: `http://localhost:5244/api/AmbulatoryEpisode/${props.episodeId}`,
            data: {
                    ...
                    <div className={styles.inputsDiv}>
                        <form>
                            <div className={styles.form_group}>
                                <label htmlFor="select_diagnosis" className={styles.label}>Назва діагнозу</label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="input_episode_name" className={styles.label}>Назва епізоду</label>
                                <input type="text" id="input_episode_name"  className='form-control' value={episodeName} onChange={changeEpisodeName} placeholder='Назва епізоду'/>
                                <p>* Назва епізоду може складатися з діагнозу (за винятком психіатричних та ВІЛ-діагнозів), а також містити іншу корисну інформацію щодо звернення пацієнта.</p>
                            </div>
                            <div className={styles.form_group}>
                                <label htmlFor="select_episode_type" className={styles.label}>Тип епізоду</label>
                                <Select 
                            ...
                            <div className={styles.container_update_btn}><button type="button" className={styles.updateBtn} onClick={updateEpisode}>Зберегти</button></div>
                        </form>
                    </div>
                </div>
                {props.children}
