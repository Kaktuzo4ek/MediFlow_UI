...
const EditDiagnosticReportModal = props => {
    ...
    const reportCategoryOptions = [
        { value: "Лікувально-діагностична процедура", label: "Лікувально-діагностична процедура"},
    ...
    const getDoctors = () => {
        ...
        axios({
            ...
            if(isFirstFill){
                for(let i = 0; i < response.data.length; i++)
                {
                    fillArray.push({value: Number(response.data[i].id), label: `${response.data[i].surname} ${response.data[i].name} ${response.data[i].patronymic}`});
                }
                fillArray.map(item => item.value === doctorId && setSelectExecunantData(item));
                fillArray.map(item => item.value === doctorId && setSelectInterpretedData(item));
    ...
    const validate = () => {
        if(props.reportId !== 0 && selectServicesData && selectReportCategoryData && conclusion !== "" && selectExectantData && selectInterpretedData)
    ...
    return(
        <div className={classNames(styles.modal_wrapper, `${props.isOpened ? styles.fadeIn: styles.fadeOut}`)} style={{...props.style}} onMouseOver={props.isOpen ? () => setData(): null}>
            <div className={styles.modal_body}>
                <div className={styles.modal_close}><h3>Редагування діагностичного звіту</h3><button onClick={props.onModalClose} className={styles.closeBtn}>×</button></div>
                <hr/>
                <div className={styles.updateSection}>
                    <div className={styles.inputsDiv}>
                        <form>
                            <div className={styles.form_group}>
                                <label htmlFor="select_services" className={styles.label}>Медична послуга</label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="select_report_category" className={styles.label}>Категорія діагностичного звіту</label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                ...
                                <textarea class="form-control" id="text_conclusion" value={conclusion} onChange={changeConclusion} placeholder="Введіть заключення"></textarea>
                            </div>
                            <div className={styles.form_group}>
                                <label htmlFor="select_execuatant" className={styles.label}>Виконавець діагностики</label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                ...
                                <label htmlFor="select_interpreted" className={styles.label}>Працівник, що інтерпретував результати</label>
                                <div className={styles.checkbox}>
                                    <input class="form-check-input" type="checkbox" value={otherInterpted} onChange={otherInterpted ? changeOtherInterpretedFalse : changeOtherInterpretedTrue} id="checkbox_other_interpreted"/>
                                    <label className={styles.label} for="checkbox_other_interpreted">ІНШИЙ</label>
                                </div>
                                </div>
                                <Select 
                                    options={!otherInterpted ? interpretedOptions : otherInterpretedOptions} 
                            ...
                            <div className={styles.container_update_btn}>
                                <button type="button" className={styles.updateBtn} onClick={validate}>Зберегти дані</button>
                            </div>
                        </form>
                    </div>
                </div>
