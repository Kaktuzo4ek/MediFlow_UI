...
const AmbulatoryEpisode = () => {
  ...
  const changeFilterBy = (event) => {
    ...
    else inputName.classList.remove(styles.visible);

    if (event.target.value === "type")
      inputType.classList.toggle(styles.visible);
    else inputType.classList.remove(styles.visible);
  ...
  const filterReferrals = (filter, arrayForFilter) => {
    switch (filterBy) {
      ...
      case "date":
        setEpisodes(
          arrayForFilter.filter(({ dateCreated }) =>
            dateCreated.toLowerCase().includes(filter.toLowerCase())
          )
        );
        break;
    }
  ...
  const getPatient = () => {
    axios({
      ...
      .then((response) => {
        setPatient(
          `${response.data.surname} ${response.data.name} ${response.data.patronymic}`
        );
      })
      .catch((error) => console.error(`Error: ${error}`));
  };
  ...
  const setEditModalAndData = (eId, eName, tObj, dId, dName) => {
    ...
    if (dId && dName) setDiagnosisObj({ value: dId, label: `${dId} ${dName}` });
    else setDiagnosisObj({ value: "", label: "" });
  };
              ...
              <div className={styles.flexSelectAndBtn}>
                <div className={styles.flexForSelects}>
                  ...
                  <input
                    ...
                    value={filter}
                    onChange={changeFilter}
                    placeholder="Введіть дата початку"
                  />
                </div>
                <div className={styles.flexButtons}>
                  <button
                    ...
                    onClick={() => filterReferrals(filter, filterEpisodes)}
                  >
                    Пошук
                  </button>
                  <button
