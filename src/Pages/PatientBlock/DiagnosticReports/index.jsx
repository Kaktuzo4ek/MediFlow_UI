...
import styles from "./diagnosticReports.module.scss";
import { useEffect } from "react";
import classNames from "classnames";
import { useState } from "react";
import CreateDiagnosticReportModal from "../../../ModalWindows/DiagnosticReport/CreateDiagnosticReport";
...
const DiagnosticReports = () => {
  ...
  const getReports = () => {
    axios({
      method: "get",
      url: "http://localhost:5244/api/DiagnosticReport",
  ...
  const deleteReport = (reportId) => {
    axios({
      method: "delete",
      url: `http://localhost:5244/api/DiagnosticReport/${reportId}`,
      params: { reportId },
    })
      .then((response) => {
        getReports();
  ...
  useEffect(() => {
    document.title = "Діагностичні звіти";
    if (userToken !== null) {
      axios({
        method: "get",
        ...
        <div className={styles.navSection}>
          <div className={styles.container}>
            <div className={styles.btnContainer}>
              ...
              >
                Cтворити діагностичний звіт
              </button>
              <button
                type="button"
                className={styles.navButtons}
                onClick={() =>
                  navigate("../doctor/medical-events/patient-episodes")
                }
              >
                Повернутися до епізодів
              </button>
            </div>
          </div>
        </div>
                  ...
                  ? reports.map((item, index) => {
                      return (
                        <tr
                          ...
                          title={
                            item.referral
                              ? `Звітом погашене скерування №${item.referral.referralPackageId}`
                              : "Діагностичний звіт зареєстровано в рамках амбулаторної взаємодії"
                          }
                        >
                          <td>{index + 1}</td>
                          <td>{item.category}</td>
