...
const EditAppointment = () => {
        ...
        axios({
            ...
            if(isFirstFill){
                for(let i = 0; i < response.data.length; i++)
                    fillArray.push({value: response.data[i].diagnosisId, label: `(${response.data[i].diagnosisCode})` +" "+ (response.data[i].diagnosisName)});
    ...
    const interactionClassOptions = [
        { value: "Амбулаторна медична допомога", label: "Амбулаторна медична допомога"}
    ];
    const [selectInteractionClassData, setSelectInteractionClassData] = useState(interactionClassOptions[0]);
    ...
    const [selectVisitingData, setSelectVisitingData] = useState(visitingOptions[0]);

    const interactionTypeOptions = [
        { value: "Взаємодія в закладі охорони здороров'я", label: "Взаємодія в закладі охорони здороров'я"},
        { value: "Консультація пацієнта засобами зв'язку", label: "Консультація пацієнта засобами зв'язку"},
        { value: "Візит за місцем постійного перебування пацієнта", label: "Візит за місцем постійного перебування пацієнта"},
        { value: "Візит за межами медичного закладу та місцем постійного перебування пацієнта", label: "Візит за межами медичного закладу та місцем постійного перебування пацієнта"},
        { value: "Проведення інтервенцій", label: "Проведення інтервенцій"},
    ];
    const [selectInteractionTypeData, setSelectInteractionTypeData] = useState(interactionTypeOptions[0]);
    ...
    const [selectPriorityData, setSelectPriorityData] = useState(priorityOptions[0]);
    ...
    const getAppointment = () => {
        axios({
            ...
            setSelectVisitingData({value: response.data.visiting, label:response.data.visiting});
            setSelectInteractionTypeData({value: response.data.interactionType, label:response.data.interactionType});
            tempArray = [];
            response.data.appointmentsAndServices.map(item => tempArray.push({value: item.service.serviceId, label: `(${item.service.serviceId}) ${item.service.serviceName}`}));
            setSelectServicesData(tempArray);
            setServicesComment(response.data.serviceComment);
            setSelectPriorityData({value: response.data.priority, label:response.data.priority});
                ...
                <div className={styles.navSection}>
                    <div className={styles.container}>
                        <div className={styles.btnContainer}>
                            <button type="button" className={styles.navButtons} onClick={() => navigate("../doctor/medical-events/patient-episodes/view-interactions")}>Повернутися до прийомів</button>
                        </div>
                    </div>
                </div>
                <div className={styles.appointmentBlock}>
                    <div className={styles.container}>
                        <form>
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="select_diagnosis" className={styles.label}>Причина звернення (ICPC-2) <span>*</span></label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="text_area_reasons_comment" className={styles.label}>Коментарі до причин</label>
                                <textarea class="form-control" id="text_area_reasons_comment" rows="3" value={reasonsComment} onChange={changeReasonsComment} placeholder="Введіть коментарі до причин"></textarea>
                            </div>
                            <div className={styles.form_group}>
                                <label htmlFor="select_interaction_class" className={styles.label}>Клас взаємодії <span>*</span></label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="select_visiting" className={styles.label}>Відвідування <span>*</span></label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="select_interaction_type" className={styles.label}>Тип взаємодії <span>*</span></label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="select_services" className={styles.label}>Медичні послуги <span>*</span></label>
                                <Select 
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="text_services_comment" className={styles.label}>Коментарі до наданих медичних послуг</label>
                                <textarea class="form-control" id="text_services_comment" rows="3" value={servicesComment} onChange={changeServicesComment} placeholder="Введіть коментарі до наданих медичних послуг"></textarea>
                            </div>
                            <div className={styles.form_group}>
                            ...
                            <div className={styles.form_group}>
                                <label htmlFor="text_treatment" className={styles.label}>Призначене лікування</label>
                                <textarea class="form-control" id="text_treatment" rows="3" value={treatment} onChange={changeTreatment} placeholder="Введіть призначене лікування"></textarea>
                            </div>
                            <div className={styles.form_group}>
