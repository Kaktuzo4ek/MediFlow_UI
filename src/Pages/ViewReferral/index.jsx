    ...
    if (episodeId !== 0) {
      ...
      axios({
        ...
        .then((response) => {
          response.data.map(
            (item, index) =>
              item.referralPackage.referralPackageId === referralPackageId &&
              setEpisode(response.data[index]),
          );
          console.log(response.data[0]);
        })
        .catch((error) => console.error(`Error: ${error}`));
  ...
  const getReferral = () => {
    axios({
      ...
      .then((response) => {
        ...
        response.data[0].referrals.map(
          (item) => item.referralId === referralId && setReferral(item),
        );
      })
      .catch((error) => console.error(`Error: ${error}`));
  };
                  ...
                  <p>
                    ...
                    <span>
                      {episode &&
                        ...
                        `(${episode.diagnosisMKX10AM.diagnosisId}) ${episode.diagnosisMKX10AM.diagnosisName}`}
                    </span>
                  </p>
                </div>
                <div className={styles.wrapper}>
                  <div className={styles.referralData}>
                    <div className={styles.referralHeader}>
                      <h3>
                        Електронне направлення: {referral.referralPackageId}
                      </h3>
                      <h3 className={styles.hForPrint}>
                        Електронне направлення
                      </h3>
                      <Image
                        src={barcodeIcon}
                        className={styles.barcodePhoto}
                      />
                    </div>
                    <div className={styles.referralInfo}>
                      <p>
                        Категорія направлення: <span>{referral.category}</span>
                      </p>
                      <p>
                        Послуга:{' '}
                        <span>{`(${referral.service.serviceId}) ${referral.service.serviceName}`}</span>
                      </p>
                      <p>
                        Джерело фінансування:{' '}
                        <span>
                          Програма державних фінансових гарантій медичного
                          обслуговування населення
                        </span>
                      </p>
                      {referral.hospitalizationDepartment && (
                        <p>
                          Відділення для госпіталізації:{' '}
                          <span>{referral.hospitalizationDepartment.name}</span>
                        </p>
                      )}
                      <p>
                        Пріоритет направлення: <span>{referral.priority}</span>
                      </p>
                      <p>
                        Дата створення:{' '}
                        <span>{`${referral.date.split('T')[0]} ${referral.date
                          .split('T')[1]
                          .slice(0, 5)}`}</span>
                      </p>
                      <p>
                        ...
                        <span>{`${
                          referral.validity.split('T')[0]
                        } ${referral.validity
                          .split('T')[1]
                          .slice(0, 5)}`}</span>
                      </p>
                      <p>
                        Статус направлення: <span>{referral.status}</span>
                      </p>
                    </div>
                    <div className={styles.referralInfoPrint}>
                      <h4>
                        <b>№</b> {referral.referralPackageId}
                      </h4>
                      ...
                      <p>
                        Пацієнт:{' '}
                        <span>{`${referral.patient.surname} ${referral.patient.name} ${referral.patient.patronymic}`}</span>
                      </p>
                      <p>
                        Дата народження пацієнта:{' '}
                        <span>
                          {referral.patient.dateOfBirth.split('T')[0]}
                        </span>
                      </p>
                      <p>
                        Категорія направлення:{' '}
                        <span>
                          <b>{referral.category}</b>
                        </span>
                      </p>
                      {referral.hospitalizationDepartment && (
                        <p>
                          Відділення для госпіталізації:{' '}
                          <span>
                            <b>{referral.hospitalizationDepartment.name}</b>
                          </span>
                        </p>
                      )}
                      <p>
                        Послуга:{' '}
                        <span>
                          <b>{`(${referral.service.serviceId}) ${referral.service.serviceName}`}</b>
                        </span>
                      </p>
                      <p>
                        Пріоритет направлення:{' '}
                        <span>
                          <b>{referral.priority}</b>
                        </span>
                      </p>
                      <p>
                        Тип епізоду: <span>{episode.type}</span>
                      </p>
                      <p>
                        Діагноз:{' '}
                        <span>{`(${episode.diagnosisMKX10AM.diagnosisId}) ${episode.diagnosisMKX10AM.diagnosisName}`}</span>
                      </p>
                      <p>
                        Лікар:{' '}
                        <span>{`${referral.doctor.surname} ${referral.doctor.name} ${referral.doctor.patronymic}`}</span>
                      </p>
                      <p>
                        Телефон лікаря:{' '}
                        <span>{referral.doctor.phoneNumber}</span>
                      </p>
                      <p>
                        Найменування закладу охорони здоров'я:{' '}
                        <span>{referral.doctor.institution.name}</span>
                      </p>
                      <p>
                        Код за ЄДРПОУ:{' '}
                        <span>{referral.doctor.institution.institutionId}</span>
                      </p>
                      <h5>
                        Номер направлення: надіслано на номер{' '}
